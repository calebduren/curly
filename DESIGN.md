...
**The Fair Comparison Rule.** Original and With Curly share the same selected typeface and container. The reading preview offers Serif / Sans serif as a separate, labeled group of pressed-state buttons. Serif is the default; the selection applies to prose, headings, italics, inspection, and replay. Source and code keep their monospace faces, and page branding retains Kyoto. Changing typeface is immediate and does not change punctuation options or source text. Optional reading styles affect the same preview independently of both choices.
...
A brief greeting runs after the logo first becomes visible, or on mouse hover / primary pointer press. The right quote squashes and crossfades into the small tilde, preserving the real font shapes. The mouth lifts by 0.03em, shifts by 0.02em, and rotates −18° toward the winking eye. CSS transitions use `cubic-bezier(0.23, 1, 0.32, 1)`: the eye closes over 180ms with a 140ms fade; the mouth turns over 280ms. The resting target returns after 320ms, with a 240ms eye release, 180ms fade, and 440ms mouth settle. The smile returns continuously to level without an extra rebound pose. Repeated pointer triggers extend the wink and retarget the current pose smoothly.

Only transform and opacity animate; there is no SVG, animation library, per-frame JavaScript, or perpetual loop. The single release timer is cleared on cleanup. Offscreen, hidden-document, and reduced-motion states stop the gesture and disable its transitions. Keyboard activation does not trigger it. The face is decorative to assistive technology; the enclosing home link retains its accessible name and focus treatment.
